...
import come.poyu.dao.impl.RequestInfoimpl;
...
@WebServlet("/jsp/user/carOperation/UserSendEmail")
public class UserSendEmailServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String Title = "交通违章网上处理申请";
        RequestInfo requestInfo = new RequestInfo();
        RequsetInfoDao requsetInfoDao = new RequestInfoimpl();
        Object query = JDBCHelper.executeScalar("select *from t_requestinfo where unrule_record=?", req.getParameter("unrule_record"));
        if (query != null) {
            out.print("您已申请,请等待处理！");
            return;
        }
        requestInfo.setEmail(receptor);
        requestInfo.setCar_number(req.getParameter("car_number"));
        requestInfo.setUnrule_record(req.getParameter("unrule_record"));
        requestInfo.setContent(req.getParameter("content"));
        int result = requsetInfoDao.add(requestInfo);
        String send = sendEmail(receptor, Title, Content);
        if (result == 1) {
            if (receptor != "" && Content != "") {
                if ( send != null) {
                    resp.sendRedirect(req.getContextPath() + "/jsp/common/email/ok.jsp");
                } else {
                    out.print("邮箱地址错误!");
                }
                //resp.sendRedirect("error.jsp");
            } else {
                resp.sendRedirect(req.getContextPath() + "/jsp/common/email/error.jsp");
            }
        } else {
            out.print("填写错误");
        }
