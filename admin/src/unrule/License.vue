<template>
  <section>
    ...
    <el-table :data="rows" style="width: 100%;overflow: auto;" :height="clientHeight" stripe border highlight-current-row v-loading="pageLoading">
      ...
      <el-table-column label="身份证号码" width="210">
        <template slot-scope="scope">
          <i class="el-icon-time"></i>
      ...
      <el-table-column label="驾驶证档案编号" width="210">
        <template slot-scope="scope">
          <i class="el-icon-time"></i>
      ...
      <el-table-column label="驾照分数" width="180" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">
      ...
      <el-table-column label="备注" width="180" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">
            <el-tag size="medium">{{scope.row.remarks == ''? '无':scope.row.remarks}}</el-tag>
          </div>
        </template>
      </el-table-column>
    ...
    <!--对话框-->
   <!-- <el-dialog :title="form && form.id ? '修改' : '新增' " :visible.sync="formVisible" :close-on-click-modal="false">
      <el-form :model="form" label-width="120px" :rules="rules" ref="form">
        <el-form-item label="驾照分数" prop="approval" >
...
-->
    ...
    <el-dialog :title="form && form.id ? '修改' : '新增' " :visible.sync="formVisible" :close-on-click-modal="false">
      ...
      </el-form> <el-form :model="form" label-width="120px" :rules="rules" ref="form">
      <el-form-item label="驾驶证档案编号" prop="licensenumber" >
        <el-input v-model="form.licensenumber" />
      </el-form-item>
    </el-form>
      <el-form :model="form" label-width="120px" :rules="rules" ref="form">
        <el-form-item label="驾照分数" prop="score" >
          <el-input v-model="form.score" />
        </el-form-item>
      </el-form>
      <el-form :model="form" label-width="120px" :rules="rules" ref="form">
...
<script>
  ...
  let handleDelete = function(index, row) {
    ...
    this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
      ...
      this.$axios.get('/api/licensemanage/remove/' + row.id).then(res => {
  ...
  let getRows = function() {
    ...
    //调用post请求
    this.$axios.post('/api/licensemanage/loadPage', params).then(res => {
      this.pageLoading = false
  ...
  let handleSubmit = function() {
    ...
    this.$refs.form.validate(valid => {
      ...
      //调用http协议
      this.$axios.post('/api/licensemanage/save', this.form).then(res => {
