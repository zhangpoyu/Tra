...
<section>
  ...
  <el-table :data="rows" style="width: 100%;overflow: auto;" :height="clientHeight" stripe border highlight-current-row v-loading="pageLoading">
    ...
    <el-table-column label="违章编号" width="180">
      <template slot-scope="scope">
        <i class="el-icon-time"></i>
        <span style="margin-left: 10px">{{ scope.row.unrule_record }}</span>
      </template>
    </el-table-column>
    <el-table-column label="违章原因" width="180">
      <template slot-scope="scope">
        <i class="el-icon-time"></i>
        <span style="margin-left: 10px">{{ scope.row.unrule_reason }}</span>
      </template>
    </el-table-column>
  ...
  <el-dialog :title="form && form.id ? '编辑' : '新增' " :visible.sync="formVisible" :close-on-click-modal="false">
    <el-form :model="form" label-width="100px" :rules="rules" ref="form">
      ...
      <el-form-item label="违章编号" prop="unrule_record">
      <el-input v-model="form.unrule_record" />
    </el-form-item>
      <el-form-item label="违章原因" prop="unrule_reason">
      <el-input v-model="form.unrule_reason" />
    </el-form-item>
    </el-form>
