...
<script>
  ...
  let handleDelete = function(index, row) {
    ...
    this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
      ...
      this.$axios.get('/api/supermanagerlogin/remove/' + row.login_name).then(res => {
  ...
  let getRows = function() {
    ...
    //调用post请求
    this.$axios.post('/api/supermanagerlogin/loadPage', params).then(res => {
      this.pageLoading = false
  ...
  let handleSubmit = function() {
    ...
    this.$refs.form.validate(valid => {
      ...
      //调用http协议
      this.$axios.post('/api/supermanagerlogin/save', this.form).then(res => {
