<template>
  <section>
    ...
    <el-table :data="rows" style="width: 100%;overflow: auto;" :height="clientHeight" stripe border highlight-current-row v-loading="pageLoading">
      ...
      <el-table-column label="车辆类型" width="180" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            <p>车辆标志: {{ scope.row.car_sign}}</p>
            <p>车辆类型: {{ scope.row.car_style }}</p>
            <div slot="reference" class="name-wrapper">
              <el-tag size="medium">{{ scope.row.car_style }}</el-tag>
            </div>
          </el-popover>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="form && form.id ? '编辑' : '新增' " :visible.sync="formVisible" :close-on-click-modal="false">
      <el-form :model="form" label-width="100px" :rules="rules" ref="form">
        ...
        <el-form-item label="车牌号" prop="car_number">
          <el-input v-model="form.car_number" />
        </el-form-item>
        <el-form-item label="车辆标志" prop="car_sign">
          <el-input v-model="form.car_sign" />
        </el-form-item>
        <el-form-item label="车辆类型" prop="car_style">
          <el-input v-model="form.car_style" />
        </el-form-item>
      </el-form>
...
<script>
  ...
  let handleDelete = function(index, row) {
    ...
    this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
      ...
      this.$axios.get('/api/owner/remove/' + row.id).then(res => {
  ...
  let getRows = function() {
    ...
    //调用post请求
    this.$axios.post('/api/owner/loadPage', params).then(res => {
      this.pageLoading = false
  ...
  let handleSubmit = function() {
    ...
    this.$refs.form.validate(valid => {
      ...
      //调用http协议
      this.$axios.post('/api/owner/save', this.form).then(res => {
